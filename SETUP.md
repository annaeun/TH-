...
## 6. 사용 방법
- 유튜브 링크를 입력하거나, 파일을 업로드(또는 드래그 앤 드롭)한 뒤 "분석 시작"을 누르면
  Gemini 1.5 Flash가 태국어 텍스트, 한국어 번역, 단어 정리표를 생성합니다.
- 유튜브 링크는 **공개(public) 영상만** 지원됩니다 (비공개/연령제한 영상은 분석 불가).
  TikTok·인스타그램 등 다른 사이트 링크는 지원되지 않으니, 해당 영상은 다운로드 후
  파일 업로드를 이용해주세요.
- 업로드 지원 형식:
  - 영상: MP4·MOV·WEBM·AVI·WMV·FLV·3GP (음성 대사를 받아적어 분석)
  - 음성: MP3·WAV·AAC·OGG·FLAC·AIFF (음성 대사를 받아적어 분석)
  - 이미지: JPG·PNG·WEBP·HEIC 등 (사진 속 태국어 텍스트를 읽어 분석 — 표지판, 메뉴판, 캡처 등)
  - 문서: PDF (문서 안의 태국어 텍스트를 읽어 분석)
  - 영상/음성이든 이미지/PDF든 결과는 동일한 형식(스크립트·번역·단어표)으로 출력됩니다.
- 각 결과 영역(스크립트/번역/단어표) 상단의 "복사하기"로 개별 복사, 맨 아래
  "전체 복사하기"로 3개 영역을 한 번에 복사할 수 있습니다. 단어표는 일반 텍스트와
  HTML 표를 함께 복사하므로, 네이버 블로그 등 리치 에디터에 붙여넣어도 표 형태가
  유지됩니다.
